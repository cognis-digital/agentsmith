...
How **agentsmith** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `agentsmith` lives in the **Agents + cognition** cluster.
...
| from | relation | to |
|---|---|---|
| `agentsmith` | JSON findings compose with | [`agentlex`](https://github.com/cognis-digital/agentlex), [`humind`](https://github.com/cognis-digital/humind), [`agentmap`](https://github.com/cognis-digital/agentmap), [`engram`](https://github.com/cognis-digital/engram) |
| `agentsmith` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `agentsmith` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `agentsmith` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the agents + cognition cluster.** `agentsmith` output feeds the next tool:
```bash
agentsmith ... --format json > out.json          # this tool's findings (see `agentsmith --help`)
...
agentsmith ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
agentsmith ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
agentsmith ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `agentsmith`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `agentsmith` slots
